...
function BookingTable({ bookings, columns, title }) {
  ...
  const filteredBookings = bookings.filter((booking) => {
    return Object.values(booking).some((value) => value.toString().toLowerCase().includes(searchTerm.toLowerCase()))
  })
              ...
              currentBookings.map((booking, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <button className="text-pink-600 hover:text-pink-900" onClick={() => handleEditBooking(booking)}>
                      Edit
                    </button>
                  </td>
                </tr>
              ...
              {columns.map((column, index) => (
                <div key={index} className="grid grid-cols-3 gap-4">
                  ...
                  <div className="col-span-2 text-sm text-gray-900">
                    {column.accessor === "status" ? (
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusClass(viewingBooking.status)}`}
                      >
                        {viewingBooking.status}
                      </span>
                    ) : (
                      viewingBooking[column.accessor]
                    )}
                  </div>
                </div>
              ))}
